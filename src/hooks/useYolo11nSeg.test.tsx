...
import { useYolo11nSeg } from "./useYolo11nSeg";
import type {
  Yolo11nSegDetection,
...
function createFakeClient(
  ...
  return {
    detect: vi.fn(async (): Promise<Yolo11nSegDetection[]> => DEFAULT_DETECTIONS),
    terminate: vi.fn(),
    ...overrides,
  };
}
...
describe("useYolo11nSeg", () => {
  it("初期状態は idle で検出結果・エラーが無い", () => {
    const { result } = renderHook(() =>
      useYolo11nSeg({ createClient: () => createFakeClient() })
    );
  ...
  it("推論が失敗すると error 状態になりエラーメッセージを保持する", async () => {
    ...
    const { result } = renderHook(() => useYolo11nSeg({ createClient: () => client }));
  ...
  it("アンマウント時に client.terminate() を呼ぶ", () => {
    ...
    const { result, unmount } = renderHook(() =>
      useYolo11nSeg({ createClient: () => client })
    );
