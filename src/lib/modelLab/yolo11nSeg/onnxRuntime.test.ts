...
// `onnxruntime-web` は実 WASM 推論を伴う重い依存のため、MobileSAM 実装
// (`../mobileSam/onnxRuntime.test.ts`) と同様に実パッケージを import せずモックする。
...
describe("createOnnxRuntimeWebRuntime (yolo11nSeg)", () => {
  ...
  it("wasm バックエンドで InferenceSession.create を呼ぶ", async () => {
    ...
    await runtime.createSession("/models/yolo11n-seg/yolo11n-seg.onnx");

    expect(mockCreate).toHaveBeenCalledWith("/models/yolo11n-seg/yolo11n-seg.onnx", {
  ...
  it("run() は Yolo11nSegTensor を ort.Tensor へ変換して session.run に渡す", async () => {
    mockRun.mockResolvedValue({});

    const { createOnnxRuntimeWebRuntime } = await import("./onnxRuntime");
    const runtime = createOnnxRuntimeWebRuntime();
    const session = await runtime.createSession("/models/yolo11n-seg/yolo11n-seg.onnx");
    await session.run({
      input: { data: new Float32Array([9, 9]), dims: [1, 2] },
    });
    ...
    >;
    expect(feeds.input.dims).toEqual([1, 2]);
    expect(Array.from(feeds.input.data)).toEqual([9, 9]);
  });

  it("run() は session.run の出力を Yolo11nSegTensor へ変換して返す", async () => {
    mockRun.mockResolvedValue({
      output: { data: new Float32Array([1, 2, 3]), dims: [1, 3] },
    });
    ...
    const result = await session.run({
      input: { data: new Float32Array([0]), dims: [1] },
    });

    expect(result.output.dims).toEqual([1, 3]);
    expect(Array.from(result.output.data)).toEqual([1, 2, 3]);
  });
});
