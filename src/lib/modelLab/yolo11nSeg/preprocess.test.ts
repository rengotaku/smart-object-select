...
describe("computeLetterboxTransform", () => {
  it("横長画像は幅を基準にスケールし、上下にパディングする", () => {
    const transform = computeLetterboxTransform(200, 100, 640);
    expect(transform.scale).toBeCloseTo(640 / 200);
    expect(transform.resizedWidth).toBe(640);
    expect(transform.resizedHeight).toBe(320); // round(100 * 3.2)
    expect(transform.padX).toBe(0);
    expect(transform.padY).toBe(Math.floor((640 - 320) / 2));
  });

  it("縦長画像は高さを基準にスケールし、左右にパディングする", () => {
    const transform = computeLetterboxTransform(100, 200, 640);
    expect(transform.scale).toBeCloseTo(640 / 200);
    expect(transform.resizedHeight).toBe(640);
    expect(transform.resizedWidth).toBe(320);
    expect(transform.padY).toBe(0);
    expect(transform.padX).toBe(Math.floor((640 - 320) / 2));
  });

  it("正方形画像はパディング無しで targetSize になる", () => {
    const transform = computeLetterboxTransform(50, 50, 640);
    expect(transform.resizedWidth).toBe(640);
    expect(transform.resizedHeight).toBe(640);
    expect(transform.padX).toBe(0);
    expect(transform.padY).toBe(0);
  ...
  it("targetSize を省略すると既定値 640 が使われる", () => {
    const transform = computeLetterboxTransform(320, 320);
    expect(transform.resizedWidth).toBe(640);
    expect(transform.resizedHeight).toBe(640);
  });
});
...
describe("mapBoxToOriginal", () => {
  ...
  it("スケールとパディングを逆変換する", () => {
    // 元画像 200x100 -> targetSize=640 -> scale=3.2, padX=0, padY=(640-320)/2=160
    const transform = {
      scale: 3.2,
      padX: 0,
      padY: 160,
    ...
    // 640空間でのボックス: x=320,y=160,w=64,h=32 (中央付近)
    const box = mapBoxToOriginal(
      { x: 320, y: 160, width: 64, height: 32 },
      transform,
      200,
      100
    );
