...
import { YOLO11N_SEG_INPUT_SIZE, YOLO11N_SEG_LETTERBOX_PAD_VALUE } from "./constants";
...
export function computeLetterboxTransform(
  width: number,
  height: number,
  targetSize: number = YOLO11N_SEG_INPUT_SIZE
): LetterboxTransform {
 ...
 * 精度チューニングは対象外のため（issue #49 スコープ）、双線形補間等は行わない
 * （MobileSAM の `resizeRgbaNearest` と同じ方針）。
 */
export function letterboxResize(
  ...
  for (let i = 0; i < targetSize * targetSize; i++) {
    output[i * 4] = YOLO11N_SEG_LETTERBOX_PAD_VALUE;
    output[i * 4 + 1] = YOLO11N_SEG_LETTERBOX_PAD_VALUE;
    output[i * 4 + 2] = YOLO11N_SEG_LETTERBOX_PAD_VALUE;
    output[i * 4 + 3] = 255;
  }
 ...
 * （`ultralytics/data/augment.py` の `ToTensor`/`img / 255` 相当）。
 */
export function toModelInputData(
  rgba: Uint8ClampedArray,
  size: number = YOLO11N_SEG_INPUT_SIZE
): Float32Array {
