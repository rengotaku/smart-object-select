...
import { createFastSamWorkerClient, type WorkerLike } from "./fastSamWorkerClient";
...
describe("createFastSamWorkerClient", () => {
  it("detect は postMessage し、result 応答の検出結果で resolve される", async () => {
    const worker = new FakeWorker();
    const client = createFastSamWorkerClient(worker, () => "req-1");
  ...
  it("応答が来ていない複数リクエストは id で相関する（順不同でも取り違えない）", async () => {
    ...
    const ids = ["r1", "r2"];
    let callIndex = 0;
    const client = createFastSamWorkerClient(worker, () => ids[callIndex++]);

    const first = client.detect(IMAGE);
    const second = client.detect(IMAGE);

    worker.emit({ id: "r2", type: "result", payload: [{ score: 0.5 }] });
    worker.emit({ id: "r1", type: "result", payload: [{ score: 0.9 }] });

    const [firstResult, secondResult] = await Promise.all([first, second]);

    expect((firstResult[0] as { score: number }).score).toBe(0.9);
    expect((secondResult[0] as { score: number }).score).toBe(0.5);
  });

  it("error 応答は対応する promise を reject する", async () => {
    const worker = new FakeWorker();
    const client = createFastSamWorkerClient(worker, () => "e1");
  ...
  it("terminate は pending 中のリクエストを reject し worker.terminate() を呼ぶ", async () => {
    const worker = new FakeWorker();
    const client = createFastSamWorkerClient(worker, () => "t1");
  ...
  it("terminate は登録済みリスナーを全て解除する", () => {
    const worker = new FakeWorker();
    const client = createFastSamWorkerClient(worker);
