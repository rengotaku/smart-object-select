...
import { createMobileSamWorkerClient, type WorkerLike } from "./mobileSamWorkerClient";
...
describe("createMobileSamWorkerClient", () => {
  it("setImage は postMessage し、result 応答で resolve される", async () => {
    const worker = new FakeWorker();
    const client = createMobileSamWorkerClient(worker, () => "req-1");
  ...
  it("応答が来ていない複数リクエストは id で相関する（順不同でも取り違えない）", async () => {
    ...
    const ids = ["r1", "r2"];
    let callIndex = 0;
    const client = createMobileSamWorkerClient(worker, () => ids[callIndex++]);
  ...
  it("error 応答は対応する promise を reject する", async () => {
    const worker = new FakeWorker();
    const client = createMobileSamWorkerClient(worker, () => "e1");

    const request = client.segmentAtPoint(0, 0);
    worker.emit({ id: "e1", type: "error", name: "MobileSamNoImageError", message: "x" });

    await expect(request).rejects.toMatchObject({
      name: "MobileSamNoImageError",
      message: "x",
    });
  });
  ...
  it("terminate は pending 中のリクエストを reject し worker.terminate() を呼ぶ", async () => {
    const worker = new FakeWorker();
    const client = createMobileSamWorkerClient(worker, () => "t1");
  ...
  it("terminate は登録済みリスナーを全て解除する", () => {
    const worker = new FakeWorker();
    const client = createMobileSamWorkerClient(worker);
