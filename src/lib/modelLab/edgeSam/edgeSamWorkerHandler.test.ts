 ...
 * `edgeSam.worker.ts` の onmessage が先行リクエストの完了を待たずに次を処理する
 * 競合状態を決定的に再現するために使う。
 */
function deferred<T>() {
...
describe("createEdgeSamWorkerHandler", () => {
  it("setImage リクエストは result 応答を返す", async () => {
    const handler = createEdgeSamWorkerHandler(createFakeRuntime());
  ...
  it("setImage 前に segmentAtPoint を送ると reject せずエラー応答を返す", async () => {
    ...
    expect((response as { name: string }).name).toBe("EdgeSamNoImageError");
  });

  it("セッションは初回リクエストで一度だけ生成される（2回目以降は使い回す）", async () => {
    const createSession = vi.fn(createFakeRuntime().createSession);
    const runtime: EdgeSamRuntime = { createSession };
    const handler = createEdgeSamWorkerHandler(runtime);
  ...
  it(
    ...
    async () => {
      ...
      const decoderRun = vi.fn((feeds: Record<string, EdgeSamTensor>) =>
      ...
      // edgeSam.worker.ts の onmessage は setImage(A) の完了を待たずに setImage(B) を
      ...
      const feeds = decoderRun.mock.calls[0][0] as Record<string, EdgeSamTensor>;
      // 画像B（3x3）の embeddings が使われること（画像Aで上書きされていないこと）
