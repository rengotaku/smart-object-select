...
// `onnxruntime-web` は実 WASM 推論を伴う重い依存のため、mobileSam/onnxRuntime.test.ts と
// 同様に実パッケージを import せずモックする（本ファイルのテストは
// createOnnxRuntimeWebRuntime() が ort.InferenceSession.create / session.run を
// どう呼ぶかだけを検証する）。
...
describe("createOnnxRuntimeWebRuntime", () => {
  ...
  it("wasm バックエンドで InferenceSession.create を呼ぶ", async () => {
    ...
    await runtime.createSession("/models/edge-sam/foo.onnx");

    expect(mockCreate).toHaveBeenCalledWith("/models/edge-sam/foo.onnx", {
  ...
  it(".mjs は public/ 配下の URL を直接 wasmPaths に渡さず、fetch してから Blob URL に変換する", async () => {
    ...
    // （MobileSAM実装で実機確認済み）、ort.env.wasm.wasmPaths.mjs には直接 URL を渡さず、
  ...
  it("2回目以降の createSession では .mjs を再取得しない（キャッシュされる）", async () => {
    ...
    const runtime = createOnnxRuntimeWebRuntime();
    await runtime.createSession("/models/edge-sam/foo.onnx");
    await runtime.createSession("/models/edge-sam/bar.onnx");

    expect(fetchMock).toHaveBeenCalledTimes(1);
  });

  it("run() は EdgeSamTensor を ort.Tensor へ変換して session.run に渡す", async () => {
    mockRun.mockResolvedValue({});

    const { createOnnxRuntimeWebRuntime } = await import("./onnxRuntime");
    const runtime = createOnnxRuntimeWebRuntime();
    const session = await runtime.createSession("/models/edge-sam/foo.onnx");
    await session.run({
  ...
  it("run() は session.run の出力を EdgeSamTensor へ変換して返す", async () => {
    mockRun.mockResolvedValue({
